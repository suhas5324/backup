...
namespace IMDB_WebApplication.Services.Implementations
{
    public class ReviewService : IReviewService
    {
        ...
        {
            if (movieId <= 0 || request == null || string.IsNullOrWhiteSpace(request.Message))
            {
                return null;
            }
            ...
            {
                Id = reviews.Count == 0 ? 1 : reviews.Max(existingReview => existingReview.Id) + 1,
                MovieId = movieId,
        ...
        {
            if (movieId <= 0 || id <= 0 || request == null || string.IsNullOrWhiteSpace(request.Message))
            {
                return null;
            }
